...
from srt.config import KEY_NEW_USER, KEY_NEW_RESUME, KEY_NEW_REQUIREMENTS, KEY_NEW_PROCESSING
...
from tests.conftest import KAFKA_TOPIC_CONSUMER_FOR_UPLOADING_DATA, producer

class TestKafka:
    @pytest.mark.asyncio
    async def test_user(self, clearing_kafka, db_session):
        data_for_kafka = {
            'user_id': 1,
    ...
    @pytest.mark.asyncio
    async def test_resume(self, clearing_kafka, db_session, create_user):
        data_for_kafka = {
            'resume_id': 1,
    ...
    @pytest.mark.asyncio
    async def test_requirements(self, clearing_kafka, db_session, create_user):
        data_for_kafka = {
            'requirements_id': 1,
            'user_id': create_user['user_id'],
    ...
    @pytest.mark.asyncio
    async def test_processing(self, clearing_kafka, db_session, create_resume_and_requirements):
        data_for_kafka = {
            'processing_id': 1,
